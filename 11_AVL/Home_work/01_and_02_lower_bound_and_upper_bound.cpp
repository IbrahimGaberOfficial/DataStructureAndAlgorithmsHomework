...
public:
    ...
    ////////////////////////
    BinaryNode* lower_bound(int target, BinaryNode* current){
        if(!current)
            return nullptr;
        //check if the answer is in the left
        // as we search for value >= target
        //in BST the right is > root
        if(target <= current->data){
            BinaryNode* answer = lower_bound(target, current->left);
            if(answer)
                return answer;
            return current;
        }
        return lower_bound(target, current->right);
    }
    pair<bool, int> lower_bound(int target){
        if(root){
            //reach the sol node
            BinaryNode* current = root;
            BinaryNode* answer = lower_bound(target, current);
            if(!answer)
...
void f1() {
    ...
    cout << endl << tree.lower_bound(50).second << endl;
	cout << tree.lower_bound(51).second << endl;
	cout << tree.lower_bound(71).second << endl;
	cout << "\n********************************\n"
		 << 10 << " " << tree.upper_bound(10).second << endl
		 << 20 << " " << tree.upper_bound(20).second << endl;
